...
Original file is located at
    https://colab.research.google.com/drive/1VOY-Ih5QUAWcWezHRbvXUua9-pydeytH
"""
...
# Loading data from data100D.npy:
...
sigma = tf.exp(tf.Variable(tf.random_normal(np.array([K, 1]), stddev= 0.05, dtype=X.dtype))) # The variance of the model.
logpi = tf.squeeze(hlp.logsoftmax(tf.Variable(tf.random_normal(np.array([K, 1]), stddev = 0.05, dtype=X.dtype)))) # The log of the pi parameter.
...
with tf.Session() as training_loop:
  ...
  for epoch in range(250):
    new_MU, train_loss, new_pred, new_sigma, new_logpi,_ = training_loop.run([MU, loss, prediction, sigma,  logpi, optimizer]) # Updates hyperparameters.

    if is_valid:
      v_logpdf = log_GaussPDF(val_data, new_MU, new_sigma) # Updated Gaussian PDF.
      v_loss = loss_function(v_logpdf, new_logpi) # Validation loss.
      valid_loss = training_loop.run(v_loss)
      validation_loss.append(valid_loss / len(val_data))

    training_loss.append(train_loss / len_data)
...
v_prediction = tf.argmax(tf.nn.softmax(log_posterior(v_logpdf, new_logpi)),1)
with tf.Session() as get_loss:
  tf.initializers.global_variables().run()
  valid_loss = get_loss.run(v_loss)
  v_pred = get_loss.run(v_prediction)
